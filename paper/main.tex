...
\begin{abstract}
Which feature family carries the human/AI signal in Japanese LLM-generated text: word choice or rhythm? Practitioner tools claim that the ``AI smell'' of Japanese text resides in rhythm (monotone sentence-length variation) rather than vocabulary, but this claim has not been tested against lexical features under a controlled design. We compare lexical and rhythmic fingerprints on a corpus of 350 texts from 7 LLMs (Claude 3 Haiku, Claude Sonnet 4, Claude Opus 4, GPT-3.5 Turbo, GPT-4o, GPT-OSS 20B, Llama 3.2 1B) against 700 human technical-blog articles. We first rebuild the excess-vocabulary statistics of our prior study on a document-occurrence basis (Laplace smoothing, frequency ratio $r$ and gap $\delta$, document-level $\chi^2$ tests with BH-FDR correction, bootstrap confidence intervals), following Kobak et al.\ (2025). The re-analysis is also a correction: 35\% of the 651 words we previously reported as significant do not survive at the document level, an artifact of treating repeated tokens within a document as independent evidence. We then measure 13 interpretable rhythm metrics, including burstiness of sentence-length sequences and a mora-based approximation of phonological rhythm. Three findings emerge. (1)~Rhythmic monotonization is real and shared: AI text shows lower burstiness (Cohen's $d=-0.96$), lower paragraph-structure variation ($d=-0.67$), and lower sentence-length variation ($d=-0.56$), with the same direction in all 7 models. (2)~Contrary to the practitioner claim, lexical features dominate human/AI discrimination: cross-validated AUC is 0.998 for lexical vs.\ 0.897 for rhythm features (paired difference $-0.101$, 95\% CI $[-0.105, -0.097]$), and the gap persists under document-length residualization and outlier-model exclusion. (3)~At the multivariate level, the two families form a two-layer fingerprint: lexical profiles identify \emph{which} model wrote a text (92\% accuracy in 7-way model identification vs.\ 51\% for rhythm), whereas rhythm features shift in a direction common to all models, with model-dependent magnitude. Rhythm forms the shared accent of machine writing; vocabulary is the signature of the individual model.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{H2 (fingerprint layering)}: rhythm fingerprints are homogeneous across models while lexical fingerprints differentiate them, forming a two-layer structure. We hypothesize \emph{directional} homogeneity rather than identical distributions: all models are expected to shift the same way on rhythm metrics, while the degree of the shift may differ.
\end{itemize}
...
\begin{enumerate}
    ...
    \item The first \textbf{empirical demonstration that lexical fingerprints dominate rhythmic fingerprints} for Japanese human/AI discrimination, established under a controlled comparison with leakage-safe within-fold feature selection, document-length residualization, and outlier sensitivity analyses.
...
Machine-generated text (MGT) detection is by now a well-developed research area; \citet{wu2025survey} organize the field into watermarking, statistical zero-shot methods, fine-tuned classifiers, and human-assisted approaches. The statistical line began with GLTR~\citep{gehrmann2019gltr}, which exposes per-token probability, rank, and entropy under a reference model. DetectGPT~\citep{mitchell2023detectgpt} introduced zero-shot detection from the observation that machine-generated text occupies negative-curvature regions of a model's log-probability function, and Binoculars~\citep{hans2024binoculars} contrasts the perplexity assigned by two closely related pretrained models, detecting over 90\% of ChatGPT-generated samples at a 0.01\% false-positive rate. These detectors answer \emph{whether} a text is machine-generated, but they return an opaque score: they do not identify which lexical choices or which rhythmic properties make a text machine-like. Interpretable alternatives built on classical stylometric features (perplexity, burstiness, lexical diversity, readability) have also begun to be examined~\citep{tarim2025detection}. Our study addresses this explanatory side of the problem: quantifying \emph{which} feature families carry the human/AI signal in Japanese.
...
H2 asks whether rhythm fingerprints are homogeneous across models while lexical fingerprints differentiate them. Homogeneity here means \emph{directional} homogeneity rather than identical distributions (Section~1); the analyses below probe degree and direction separately. We operationalize the question at two levels, using the 350 AI documents.
...
\toprule
 & Word & $r$ & $\delta$ & AI/H docs & $q_{\text{FDR}}$ \\
\midrule
\multirow{10}{*}{by $r$}
...
The picture reverses when features act together. Lexical profiles identify the generating model: a multinomial classifier over document-occurrence features reaches $92.1\% \pm 3.8\%$ accuracy in the 7-way task (macro-F1 0.920; chance 14.3\%), while the 13 rhythm metrics manage $50.9\% \pm 6.5\%$ (macro-F1 0.495). Excluding Llama (6-way, chance 16.7\%): lexical 90.8\%, rhythm 50.2\%. Individually weak word indicators are collectively a near-unique model signature; rhythm metrics, individually well-separated in degree, do not accumulate into one.
...
\paragraph{H2 verdict.} The hypothesis splits by level of operationalization. At the single-feature level, strict distributional homogeneity fails: every rhythm metric differs across models in \emph{degree} (all permutation $p<0.005$). At the multivariate level the layering holds: lexical profiles identify the model (92\%), rhythm profiles largely do not (51\%). The result thus matches the directional form of the hypothesis stated in Section~1: the rhythm shift is \emph{directionally} homogeneous (all 7 models are more monotone than humans on the core metrics) while its magnitude is model-dependent; the lexical profile is the model-specific fingerprint.

\section{Discussion}

\subsection{A Two-Layer Fingerprint Structure}

The results organize into two layers, summarized in Figure~\ref{fig:layers}. \textbf{Layer 1 (shared machine accent)}: rhythmic monotonization (suppressed burstiness, uniform paragraph structure, compressed sentence-length variation) points the same way in all 7 models and supports human/AI discrimination at AUC $\approx 0.90$ with 13 interpretable features. \textbf{Layer 2 (model signature)}: the lexical profile, which identifies the generating model at 92\% accuracy, plus sentence-ending rhetoric, which disperses across models more than any core rhythm metric. Human/AI discrimination is best served by vocabulary (AUC 0.998), which aggregates both layers; model attribution is served almost solely by layer 2. Compactly: \emph{what} is written tells you which model; \emph{how} it is written tells you a machine wrote it.

\begin{figure}[H]
\centering
\begin{tikzpicture}[
  font=\small,
  topbox/.style={draw, rounded corners=2pt, align=center, inner sep=7pt, fill=gray!8},
  layerbox/.style={draw, rounded corners=3pt, align=left, inner sep=9pt, text width=6.6cm, anchor=north},
  arrow/.style={-{Latex[length=2.5mm]}, thick},
  lab/.style={font=\small\itshape, align=center}
]
\node[topbox] (doc) at (0, 2.9) {A Japanese document\\(350 AI from 7 LLMs / 696 human)};
\node[layerbox] (rhythm) at (-3.95, 1.0) {%
  \emph{how it is written}\\[2pt]\textbf{Layer 1: shared machine accent}\\[3pt]
  13 rhythm metrics: burstiness, sentence-length CV, paragraph-structure CV, \ldots\\[3pt]
  Direction shared by \emph{all 7 models}; magnitude model-dependent ($d=-0.59$ to $-1.72$)\\[3pt]
  Human/AI AUC 0.897; 7-way model ID 51\%\\[3pt]
  $\Rightarrow$ tells you \emph{a model} wrote it};
\node[layerbox] (lex) at (3.95, 1.0) {%
  \emph{what is written}\\[2pt]\textbf{Layer 2: model signature}\\[3pt]
  Document-occurrence lexical profile, plus sentence-ending rhetoric\\[3pt]
  Model-specific choices: 纏め, 於く, 用いる range from 0\% to 98\% across models\\[3pt]
  Human/AI AUC 0.998; 7-way model ID 92\%\\[3pt]
  $\Rightarrow$ tells you \emph{which model} wrote it};
\draw[arrow] ([xshift=-1.2cm]doc.south) -- (rhythm.north);
\draw[arrow] ([xshift=1.2cm]doc.south) -- (lex.north);
\end{tikzpicture}
\caption{The two-layer fingerprint structure of Japanese LLM-generated text. Rhythm shifts in a direction common to all 7 models and separates human from AI text, but barely distinguishes the models from each other; the lexical profile does both, and is the only family that identifies the generating model. Numbers from Tables~\ref{tab:rhythm}--\ref{tab:ratios}.}
\label{fig:layers}
\end{figure}
...
\subsection{What AUC 0.998 Does and Does Not Mean}

An AUC of 0.998 invites skepticism, and it should. The number is a within-corpus separation between freely composed human articles and fixed-prompt generations on shared themes; topic mix, prompt framing, and register all differ between the classes (Section~\ref{sec:limitations}). It does not imply a deployable detector: an operational detector would face adversarial paraphrase, mixed human--AI authorship, and domain shift, none of which this design tests. What the number does establish is the comparative point. Both feature families face identical splits, identical confounds, and identical evaluation, so the wide and never-reversed lexical advantage over rhythm is internally valid even where the absolute AUCs are inflated by corpus construction.

The same caution applies with more force to rhythm's headline 0.897. Document length alone reaches AUC 0.811 on this corpus (AI documents are shorter), and residualized rhythm does not exceed the length-only baseline (Table~\ref{tab:aucdiff}). A substantial share of the univariate rhythm signal therefore rides on document-length differences; the length-independent remainder is real (the residualized features are orthogonal to length yet reach 0.810) but should not be equated with the headline number. Lexical discrimination, by contrast, barely moves under length residualization (0.998 $\to$ 0.974).

\subsection{Correcting Our Own Baseline}

This study's lexical layer began as an audit of our prior work, and the audit found a real defect: token-level $\chi^2$ tests treat within-document repetition as independent evidence, and 35\% of the 651 words we previously reported---about half of the AI-excess side---do not survive document-based re-analysis. We prefer stating this plainly over defending the earlier number: the corrected list (237 excess words surviving; 797 significant words in the full re-analysis) is smaller and better-founded, and the failure cases (``Hash'': 402 tokens, 2 documents) illustrate why document occurrence is the right unit for excess-vocabulary claims in any language. The correction also sharpens the cross-linguistic result: the $r$/$\delta$ complementarity of \citet{kobak2025excess} (ratio for rare AI-particular words, gap for high-frequency register words) emerges in Japanese only once the statistics are computed at the document level. One warning label belongs on the corrected list itself: the highest-ratio items include ASCII fragments (NF, Found, Internal, Foreign) produced by MeCab segmentation of English spans embedded in Japanese text. These are tokenizer artifacts rather than linguistic findings. We flag them in Table~\ref{tab:topwords} instead of silently excluding them, and no conclusion in this paper rests on them; the substantive $r$-side pattern is carried by genuine katakana loanwords such as スクリプティング and デプロイメント.
